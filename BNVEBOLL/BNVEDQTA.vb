...
Public Class FRMVEDQTA
  ...
  Public dsBoll As DataSet                  'dataset di BNVEBOLL
  Public bVisDoc As Boolean = False

  Private components As System.ComponentModel.IContainer

  Private Sub InitializeComponent()
    Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FRMVEDQTA))
  ...
  Public Overridable Sub InitEntity(ByRef cleBoll As CLEVEBOLL, ByRef dtrCorpo As DataRow)
    oCleBoll = cleBoll
    dtrEc = dtrCorpo
...
#Region "Eventi di Form"
  Public Overridable Sub FRMORDQTA_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Dim i As Integer = 0
    Dim l As Integer = 0
    Dim strNomePrimaColonnaVisibile As String = ""
    ...
    Try
          ...
          For i = 0 To grvTco.Columns.Count - 1
            ...
            If l <> 0 Then
              ...
              Else
                ...
                If strNomePrimaColonnaVisibile = "" Then strNomePrimaColonnaVisibile = grvTco.Columns(i).Name
              End If
            End If
          Next
        End If
      ...
      grvTco.Focus()
      grvTco.FocusedColumn = grvTco.Columns(strNomePrimaColonnaVisibile)
    Catch ex As Exception
      '-------------------------------------------------
      Dim strErr As String = CLN__STD.GestError(ex, Me, "", oApp.InfoError, oApp.ErrorLogFile, True)
      '-------------------------------------------------
  ...
  Public Overridable Sub tlbDispVariante_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles tlbDispVariante.ItemClick
    ...
    Try
      If NTSCInt(dtrEc!ec_magaz) = 0 Then
        oApp.MsgBoxErr(oApp.Tr(Me, 128577269702812500, "Magazzino sulla riga del documento non impostato"))
        Return
      End If

      'la chiamata è identica a quella di frmBoll.tlbVisDispVariante_ItemClick
      '------------------------------
  ...
  Public Overridable Sub grvTco_NTSFocusedRowChanged(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles grvTco.NTSFocusedRowChanged
    ...
    Try
      ...
      If grvTco.NTSGetCurrentDataRow!ec_tipo.ToString <> "Q" Then
        grvTco.Enabled = False
      Else
        If bVisDoc = False Then GctlSetVisEnab(grvTco, False) 'posso modificare le quantità solo se non sono in 'visualizzazione documento intero'
      End If

      '-------------------------
      'carico l'intestazione delle colonne
      If bVisDoc Then
        dtrT = dsBoll.Tables("CORPO").Select("ec_riga = " & NTSCInt(grvTco.NTSGetCurrentDataRow!ec_riga).ToString)
...
#End Region
  ...
  Public Overridable Function Apri() As Boolean
    ...
    Try
      ...
      dsDqta.Tables.Add(dsBoll.Tables("CORPOTC").Clone())
      ...
      If Not dtrEc Is Nothing Then
        ...
        dtrT = dsBoll.Tables("CORPOTC").Select("ec_riga = " & NTSCInt(dtrEc!ec_riga).ToString)
      Else
        'chiamato per visualizzare l'interno documento 
        dtrT = dsBoll.Tables("CORPOTC").Select()
      End If
      If dtrT.Length > 0 Then
      ...
      If bVisDoc Then
        ...
        For Each dtrT1 As DataRow In dsDqta.Tables("TEMPTCO").Rows
          dtrT2 = dsBoll.Tables("CORPO").Select("ec_riga = " & NTSCInt(dtrT1!ec_riga).ToString)
      ...
      For l = 0 To dsDqta.Tables("TEMPTCO").Rows.Count - 1
        ...
        If bVisDoc = False Then
          ...
          With dsDqta.Tables("TEMPTCO").Rows(dsDqta.Tables("TEMPTCO").Rows.Count - 1)
            !ec_tipo = "P" 'Disimpegnata
            !ec_caption = "Qta Disimpegnata"
  ...
  Public Overridable Function Salva() As Boolean
    ...
    Try
      ...
      If oCleBoll.dttET.Rows(0)!et_tipork.ToString = "T" Then
        If oCleBoll.dttECIMP.Select("ec_prriga = " & NTSCInt(dtrEc!ec_riga).ToString).Length > 0 Or _
           oCleBoll.dttLAVCENT.Select("lce_riga = " & NTSCInt(dtrEc!ec_riga).ToString).Length > 0 Then
          oApp.MsgBoxErr(oApp.Tr(Me, 128587650648906250, "Una volta inserite righe negli scarichi di produzione e/o nelle lavorazioni collegate non è più possibile cambiare le quantità dell'articolo da produrre"))
      ...
      'Controllo che non sia stata superata la quantità  disimpegnata
      If Not oCleBoll.bConsentiEvasSuperioreTC Then
        dtrRow = dsDqta.Tables("TEMPTCO").Select("ec_tipo = 'E'")
        If dtrRow.Length > 0 Then
          For i = 1 To 24
            If NTSCDec(dsDqta.Tables("TEMPTCO").Rows(0)("ec_quant" & i.ToString.PadLeft(2, "0"c))) > NTSCDec(dtrRow(0)("ec_quant" & i.ToString.PadLeft(2, "0"c))) Then
              oApp.MsgBoxErr(oApp.Tr(Me, 129350659821143087, "Ci sono delle taglie con quantità per taglia che sono superiori alle quantità da evadere"))
              Return False
            End If
          Next
        End If
      ...
      '----------------------------
      'trovo la riga di CORPOTC di veboll su cui sto lavorando
      dtrT = dsBoll.Tables("CORPOTC").Select("ec_riga = " & NTSCInt(dtrEc!ec_riga).ToString)
      ...
      Next
      'dsBoll.Tables("CORPOTC").AcceptChanges() 'non farlo: diversamente se faccio 'ripristina riga da veboll non posso ripristinare il dettaglio TCO
  ...
  Public Overridable Function CalcoloResidui() As Boolean
    Dim dttTmp As New DataTable
    Dim dtrT() As DataRow
    Dim i As Integer
    Try
      ...
      'CALCOLO DEI RESIDUI (non devo permettere di evadere più del residuo)
      If NTSCInt(dtrEc!ec_oranno) <> 0 Then
        oCleBoll.TCOOrdiniCollegati(dtrEc, dttTmp)

        If dttTmp.Rows.Count <> 0 Then
          dsDqta.Tables("TEMPTCO").Rows.Add(dsDqta.Tables("TEMPTCO").NewRow())
          With dsDqta.Tables("TEMPTCO").Rows(dsDqta.Tables("TEMPTCO").Rows.Count - 1)
            !ec_tipo = "E" 'Disimpegnata
            !ec_caption = "Qta da Evadere"
            !ec_riga = dtrEc!ec_riga
            For i = 1 To 24
              dsDqta.Tables("TEMPTCO").Rows(dsDqta.Tables("TEMPTCO").Rows.Count - 1)("ec_quant" & i.ToString.PadLeft(2, "0"c)) = _
                            NTSCDec(dttTmp.Rows(0)("mo_quant" & i.ToString.PadLeft(2, "0"c))) - _
                            NTSCDec(dttTmp.Rows(0)("mo_quapre" & i.ToString.PadLeft(2, "0"c))) - _
                            NTSCDec(dttTmp.Rows(0)("mo_quaeva" & i.ToString.PadLeft(2, "0"c)))
            Next
          End With
        End If

        If Not oCleBoll.bNew Then
          'Siamo in modifica documento
          'dobbiamo sommare le quantità movimentate originarie!
          dtrT = dsBoll.Tables("CORPOTC").Select("ec_riga = " & NTSCInt(dtrEc!ec_riga).ToString)

          If dtrT.Length > 0 Then
            For i = 1 To 24
              dsDqta.Tables("TEMPTCO").Rows(dsDqta.Tables("TEMPTCO").Rows.Count - 1)("ec_quant" & i.ToString.PadLeft(2, "0"c)) = _
                            NTSCDec(dsDqta.Tables("TEMPTCO").Rows(dsDqta.Tables("TEMPTCO").Rows.Count - 1)("ec_quant" & i.ToString.PadLeft(2, "0"c))) + _
                            NTSCDec(dtrT(0)("ec_quant" & i.ToString.PadLeft(2, "0"c)))
            Next
          End If
        End If
      End If
  ...
  Public Overridable Sub grvTco_NTSCellValueChanged(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles grvTco.NTSCellValueChanged
    Dim dtrRow() As DataRow
    Try
      'Controllo che non sia stata superata la quantità  disimpegnata
      dtrRow = dsDqta.Tables("TEMPTCO").Select("ec_tipo = 'E'")
      If oCleBoll.bConsentiEvasSuperioreTC = False Then
        If dtrRow.Length > 0 Then
          If NTSCDec(grvTco.GetDataRow(e.RowHandle)(e.Column.Name)) > NTSCDec(dtrRow(0)(e.Column.Name)) Then
            oApp.MsgBoxErr(oApp.Tr(Me, 129350705463392912, "La quantità per taglia - |" & e.Column.Caption & "| - indicata ('|" & _
                                                           NTSCDec(grvTco.GetDataRow(e.RowHandle)(e.Column.Name)) & "|')" & vbCrLf & _
                                                           "non può essere superiore a ('|" & NTSCDec(dtrRow(0)(e.Column.Name)) & "|')."))

            grvTco.GetDataRow(e.RowHandle)(e.Column.Name) = 0
          End If
        End If
      End If
    Catch ex As Exception
